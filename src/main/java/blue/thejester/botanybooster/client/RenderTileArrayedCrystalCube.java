...
public class RenderTileArrayedCrystalCube extends TileEntitySpecialRenderer<TileArrayedCrystalCube> {
	...
	@Override
	public void render(@Nonnull TileArrayedCrystalCube cube, double d0, double d1, double d2, float f, int digProgress, float unused) {
		...
		if (cube != null) {
			...
			try {
				entityItemAge.setInt(entity, ClientTickHandler.ticksInGame);
		...
		if(!stack.isEmpty()) {
			GlStateManager.pushMatrix();
			float s = stack.getItem() instanceof ItemBlock ? 1.45F : 1.2F;
	...
	private void renderAnimatedModel(TileArrayedCrystalCube te, double x, double y, double z, float partialTick) {
