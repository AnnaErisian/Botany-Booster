...
import blue.thejester.botanybooster.item.BaseItem;
...
/**
 * The majority of the logic here is equivalent to the ItemBauble class from Botania
 * And as such was written by Vazkii
 */
@Mod.EventBusSubscriber(modid = BotanyBooster.MODID)
public abstract class BaubleBaseItem extends BaseItem implements IBauble, ICosmeticAttachable, IPhantomInkable {
    ...
    // to do there is still some weirdness going on when dying/returning in the End, figure that out
    @SubscribeEvent
    public static void onDeath(LivingDeathEvent evt) {
        if(!evt.getEntityLiving().world.isRemote
            ...
            for(int i = 0; i < inv.getSlots(); i++) {
                ...
                if (!stack.isEmpty() && stack.getItem().getRegistryName().getNamespace().equals(BotanyBooster.MODID)) {
                    ((BaubleBaseItem) stack.getItem()).onUnequipped(stack, evt.getEntityLiving());
                }
            }
        }
    }
            ...
            for(int i = 0; i < baubles.getSlots(); i++) {
                if(baubles.isItemValidForSlot(i, toEquip, player)) {
                    ...
                    if(stackInSlot.isEmpty() || baubleInSlot == null || baubleInSlot.canUnequip(stackInSlot, player)) {
                        ...
                        PlayerHelper.grantCriterion((EntityPlayerMP) player, new ResourceLocation(BotanyBooster.MODID, "main/bauble_wear"), "code_triggered");
